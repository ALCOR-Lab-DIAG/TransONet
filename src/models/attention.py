...
def attention_pool(tensor, pool, hw_shape, has_cls_embed=True, norm=None):
    ...
    if tensor_dim == 4:
        ...
        #print("dim 3 tensor:"+str(tensor.shape))
    ...
    #print("attention pool x shape:"+str(tensor.shape))
    return tensor, hw_shape
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, hw_shape):
        ...
        if self.pool_first:
            ...
            #x = x.reshape(B, H, W, fold_dim, -1).permute(0, 3, 1, 2, 4)
            q = k = v = x
        else:
            ...
            qkv = (
                ...
                #self.qkv(x).reshape(B, H, W, 3, self.num_heads, -1).permute(3, 0, 4, 1, 2, 5)
            )
